...
import { getGlobalMeta, getPosts } from "@/lib/db";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...mapMetaToMetadata(meta, "Статьи невролога"),
    ...
    <main className="mx-auto max-w-6xl px-5 pb-20 pt-12">
      <header className="mb-7">
        ...
        <p className="mt-3 max-w-3xl text-slate-700">
          Практические рекомендации, обзор современных подходов и ответы на частые
          вопросы пациентов.
        </p>
      </header>
      ...
      <section className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-5">
        {posts.map((post) => (
          <Link key={post.id} href={`/posts/${post.slug}`} prefetch={false}>
